...
class QlcEngine extends EventEmitter {
  ...
  start() {
    ...
    // windowsHide: exe 本身自己已经是 CONSOLE 子系统的程序，主进程 v1.2.2 起
    // 又改成了没有控制台的 WINDOWS 子系统——没有这个选项时，Windows 会在每次
    // spawn（包括 reload() 时的重启）单独给它分配一个新控制台窗口，表现为
    // "执行任务的时候黑框一闪而过"，不加这个选项治不了根，见 2026-09-14 用户
    // 反馈。只影响窗口显示，不影响下面 stdout/stderr 的管道读取。
    this.proc = spawn(this.binaryPath, args, { env: process.env, windowsHide: true });
