...
<body>
  ...
  <div class="card" id="pairingCard" style="display:none">
    ...
    <div class="row">
      <button id="pairingRetry" class="secondary">🔄 没收到？重新获取验证码</button>
    </div>
    <div class="hint" id="pairingRetryHint"></div>
  </div>
...
<script>
  ...
  function render() {
    ...
    // version 不通过 window.native.getVersion() 主动拉的方式兜底了（见下面
    // DOMContentLoaded 的注释）——每次 Node 推 state 过来（含每 3 秒一次的
    // 轮询）都顺带带上版本号，这里只要收到过一次真实 state 就会自动补上，
    // 不会一直空白。
  ...
  // 万一管理员的 Telegram 没收到、或者验证码等太久过期/输错太多次被锁了，
  // 不用重启整个程序，点一下就能在当前连接上再要一份新码。
  $("pairingRetry").onclick = () => {
    window.native.requestPairingCode();
    $("pairingInput").value = "";
    $("pairingRetryHint").textContent = "已重新申请，请查看管理员 Telegram 收到的新验证码";
    setTimeout(() => { $("pairingRetryHint").textContent = ""; }, 5000);
  };
...
function startUI(engine) {
  ...
  webview.expose("native", {
    ...
    requestPairingCode: async () => { engine.requestPairingCode(); },
    setName: async (name) => {
