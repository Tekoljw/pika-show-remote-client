...
async function applyUpdate(downloadUrl, onProgress) {
  ...
  onProgress?.("正在下载新版本...");
  // 下载给一个硬超时——之前完全没有，网络卡住的话这里会无限挂着，界面上
  // 看起来就是"点了没反应"，见 2026-09-14 用户反馈的"卡住不动"。90s 对
  // ~100MB 的文件来说足够宽松，正常网络几秒到几十秒就该完成。
  ...
  try {
    ...
    throw new Error(e.name === "AbortError" ? "下载超时（90秒），请检查网络后重试" : `下载失败: ${e.message}`);
  } finally {
  ...
  // 用 ping 轮询而不是 timeout 命令——2026-09-14 用户实测到"点击热更新后
  // 无限弹命令行"：v1.2.2 起主进程改成没有控制台的 WINDOWS 子系统之后，
  // 这个 helper.bat 本身也是用 stdio:"ignore" 起的、没有真实控制台，
  // Windows 自带的 timeout.exe 在这种情况下会直接报错"不支持输入重定向"
  // 退出，导致 errorlevel 判断失真、循环脱缰疯转，每次循环里 tasklist/
  // find 等子命令反复起停，表现为控制台窗口一闪一闪个没完。ping 到
  // 127.0.0.1 不依赖任何控制台/键盘输入，是这个场景的标准替代写法。
  // 另外加一道硬上限（30 次，约 30 秒）兜底：不管什么原因导致 PID 迟迟
  // 不消失，最多等 30 秒就强制往下走，不会再无限循环。
