 ...
 *
 * ⚠️ 2026-09-14 起去持久化（服务端同步改动，见 agent-server.js 头部注释和
 * 项目记忆 reference_pc_ephemeral_connection.md）：
 *   1. 不再保存/读取 device_token，也没有 "auth" 这个消息类型——每次连接
 *      （含首次启动和任何原因的重连）都走 request_pairing_code，需要真人
 *      在 Agent 程序里重新输入一次验证码，没有"记住我"。
 *   2. "已连接"不再是收到一次 pc_info 就永久为真——加了客户端侧的心跳超时
 *      检测：收到服务端任意消息就刷新时间戳，超过阈值没收到任何消息（说明
 *      握手可能成功但通信已经死了）就主动断开触发重连，见 _lastServerMessageAt。
 */
...
class RemoteEngine extends EventEmitter {
  ...
  /** GUI"重新获取验证码"按钮——不需要断开重连，直接在当前这条 WS 连接上
   *  再请求一次。用途：管理员的 Telegram 没收到、验证码等太久过期了、
   *  或者单纯输错太多次被锁了，都不用重启整个程序，点一下就能拿到新码
   *  （服务端 request_pairing_code 本来就是"每次调用都发一份新码"，见
   *  agent-server.js，不需要先失效旧码）。连接本身断了的情况走"连接"
   *  按钮（reconnect()），那条路径本来就会在重新连上时自动请求一次。 */
  requestPairingCode() {
    this._send({
  ...
  _connect() {
    ...
    this.ws.on("open", () => {
      ...
      // 不再有 device_token/自动认证这回事——每次连接都要真人重新输入一次
      // 验证码，见文件头注释第 1 条。
      this.log("正在请求验证码（请让管理员查看 Telegram）");
      this._send({
        type: "request_pairing_code",
        mac_address: this.macAddress,
        hostname: require("os").hostname(),
